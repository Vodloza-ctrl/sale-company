/**
 * sale.co.zw — payments Worker (SELF-CONTAINED, no imports)  v1.2
 * ---------------------------------------------------------------------------
 * SINGLE SOURCE OF TRUTH for Paynow. The auth Worker (app.sale.co.zw)
 ...
 *
 * RETURN_URL should be set to:  https://www.sale.co.zw/checkout/
 * RESULT_URL should be set to:  https://api.sale.co.zw/paynow/result
 */
...
async function handlePay(request, env) {
  ...
  const reference = `SCZ-${crypto.randomUUID().replace(/-/g, "")}`;
  const email = typeof body.email === "string" && body.email ? body.email : "noreply@sale.co.zw";
  ...
  const fields = [
    ...
    ["additionalinfo", `sale.co.zw ${purpose} (${currency})`],
    ["returnurl", returnUrl],
...
async function confirmPaid(env, payment) {
  ...
  try {
    ...
    if (site?.draft_subdomain) await caches.default.delete(new Request(`https://${site.draft_subdomain}.sale.co.zw/`));

/**
 * sale.co.zw — Renewal Cron Worker  v2.0
 * -------------------------------------------
...
export default {
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/health")
      return json({ ok: true, service: "sale-cozw-renewal-cron", version: "2.0" });
...
function buildMessage(event, site, extra) {
  ...
  const url     = slug ? `https://${slug}.sale.co.zw` : "your website";
  const renewUrl = "https://app.sale.co.zw/dashboard/customer.html";

  switch (event) {
    case "grace_started":
      return (
        `⚠️ Your sale.co.zw subscription for *${name}* has expired.\n\n` +
    ...
    case "renewal_reminder":
      return (
        `💡 Reminder: your sale.co.zw subscription for *${name}* expires in ` +

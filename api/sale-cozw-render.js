/**
 * sale.co.zw -- Render Worker v10.29 + Booking Engine Step 2
 *
 ...
 *     availability. Talks directly to bookings.sale.co.zw (confirmed
...
async function getTemplate(templateId, env) {
  const origin = (env.PAGES_ORIGIN || 'https://www.sale.co.zw').replace(/\/$/, '');
...
async function handlePublic(request, env, slug, customDomain) {
  ...
  try {
    ...
    if (isSelfContained) {
      ...
      if (isPreview) {
        const banner = `<div style="position:fixed;bottom:0;left:0;right:0;z-index:9999;background:linear-gradient(135deg,#1a1a2e,#e94560);color:#fff;text-align:center;padding:.75rem;font-size:.85rem;font-weight:600">Preview mode -- <a href="https://app.sale.co.zw" style="color:#fff;text-decoration:underline">Go to dashboard</a> to publish</div>`;
...
async function checkAddonActive(env, siteId, addonType) {
  ...
  try {
    ...
    return resp.ok; // 200 = active, 402 = inactive (see sale-orders-worker.js)
  } catch (err) {
...
async function buildTemplateExtras(c, site, config, env) {
  ...
  if (phone) {
    ...
    extras.wa_href_general  = `https://wa.me/${num}?text=${encodeURIComponent('Hello, I found you on sale.co.zw!')}`;
...
<\/script>`;
  ...
  if (templateId === 'boutique-fashion' || templateId === 'boutique' || templateId === 'fashion') {
    ...
    if (waNum) {
      extras.wa_href_general = `https://wa.me/${waNum}?text=${encodeURIComponent('Hello, I found you on sale.co.zw!')}`;
  ...
  if (templateId === 'fashion-retail') {
    ...
    const waNum    = rawPhone.startsWith('263') ? rawPhone : rawPhone ? '263' + rawPhone.replace(/^0/, '') : '';
    extras.whatsapp_clean  = waNum;
    extras.wa_href_general = waNum ? `https://wa.me/${waNum}?text=${encodeURIComponent('Hello, I found you on sale.co.zw!')}` : '#';
  ...
  if (templateId === 'hardware-store' || templateId === 'hardware' || templateId === 'retail-hardware') {
    ...
    extras.footer_credit        = c.footer_credit        || 'Built with sale.co.zw';
...
<div class="wcz-mobile-menu" id="wcz-mobile-menu">
  ...
  const previewBanner = isPreview
    ? `<div style="position:fixed;bottom:0;left:0;right:0;z-index:9999;background:linear-gradient(135deg,#1a1a2e,#e94560);color:#fff;text-align:center;padding:.75rem;font-size:.85rem;font-weight:600">Preview mode -- <a href="https://app.sale.co.zw" style="color:#fff;text-decoration:underline">Go to dashboard</a> to publish</div>`
    : '';
...
function wa(phone, msg) {
  ...
  const m = encodeURIComponent(msg || 'Hello, I found you on sale.co.zw');
...
// rooms. Talks directly to sale-bookings-worker.js (confirmed live via
// the Worker Route on bookings.sale.co.zw -- no service binding needed
...
.wcz-book-loading{text-align:center;padding:30px 0;font-size:.8rem;opacity:.5}
  ...
  const js = `<script>(function(){
'use strict';
var WCZ_BOOK_API = "https://bookings.sale.co.zw";
...
<html lang="en"><head><meta charset="UTF-8"><title>Not Found - sale.co.zw</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
...
</head><body><h1>404</h1><p>This site isn't available.</p><a href="https://sale.co.zw">Get your own site &rarr;</a></body></html>`,

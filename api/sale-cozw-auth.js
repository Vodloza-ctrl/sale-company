/**
 * sale.co.zw — Auth + Dashboard API Worker  v5.4
 * Deploy as: sale-cozw-auth
 * Custom domain: app.sale.co.zw
 *
 ...
 *   at api.sale.co.zw. Only this Worker's version was actually wired
 ...
 *   https://api.sale.co.zw/paynow/result directly (update this in your
 ...
 *   Worker) to be byte-identical, plus an app.sale.co.zw Worker Route
 ...
 *   preview_url pointing at <slug>.sale.co.zw/?preview_token=<token>.
 *   That URL hits the render Worker's EXISTING wildcard route
 *   (*.sale.co.zw/*) — the same one that already serves public sites
 ...
 *
 * Bindings: DB (D1), ASSETS (R2 bucket: sale-cozw-assets),
 *           AI_WORKER (Service binding -> sale-cozw-ai) [optional —
 *             falls back to AI_WORKER_URL fetch if not bound],
 *           PAYMENTS_WORKER (Service binding -> sale-cozw-payments)
 *             [optional — falls back to PAYMENTS_API_URL fetch if not bound],
 *           RENDER_WORKER (Service binding -> sale-cozw-render) [legacy, unused]
 ...
 *           AI_WORKER_URL, PAYMENTS_API_URL (default https://api.sale.co.zw),
...
// Dashboard HTML lives on Pages at www.sale.co.zw/dashboard/
// app.sale.co.zw is the Worker's custom domain — redirecting /dashboard/* there
// would loop back into this Worker. Use www instead.
const PAGES_DASHBOARD = "https://www.sale.co.zw/dashboard";
...
export default {
  async fetch(request, env) {
    ...
    // ── Health check ─────────────────────────────────────────────────────────
    if (path === "/health")
      return respond({ ok: true, service: "sale-cozw-auth", version: "5.4" }, 200, origin);

    // NOTE: /preview/* proxy removed in v5.0 — preview now goes directly to
    // <slug>.sale.co.zw/?preview_token=... via the render Worker's
...
async function createEmailRoute(request, env, origin, siteId) {
  ...
  if (dup) return jsonResp({ error: "duplicate_route", message: `${localPart}@sale.co.zw is already set up.` }, 409, origin);
  const plan  = site.plan || "starter";
    ...
    try {
      await fetch(
        ...
        { method: "POST", headers: { "Authorization": "Bearer " + env.CF_API_TOKEN, "Content-Type": "application/json" },
          body: JSON.stringify({ name: `${localPart} → ${dest}`, enabled: true, matchers: [{ type: "literal", field: "to", value: `${localPart}@sale.co.zw` }], actions: [{ type: "forward", value: [dest] }] }) }
      );
    } catch {}
  }
  return jsonResp({
    ...
    message: cfErr
      ...
      : `We've sent a verification email to ${dest}. Click the link in that email to activate ${localPart}@sale.co.zw.`,
    address: `${localPart}@sale.co.zw`,
...
function buildDnsInstructions(hostname, cf) {
  ...
  // 1. CNAME their domain → sale.co.zw (routes traffic through CF for SaaS)
...
// Base URL of the standalone payments Worker (api.sale.co.zw).
// Override with the PAYMENTS_API_URL var if you ever move it.
function paymentsApiBase(env) {
  return (env.PAYMENTS_API_URL || "https://api.sale.co.zw").replace(/\/+$/, "");
}
...
// GET /pay/status, so the dashboard never has to call api.sale.co.zw
...
async function getPreviewToken(request, env, origin, id) {
  ...
  const previewUrl = `https://${site.draft_subdomain}.sale.co.zw/?preview_token=${token}`;
...
async function getUploadUrl(request, env, origin, id) {
  ...
  const bucket  = env.R2_BUCKET_NAME || "sale-cozw-assets";
  ...
  const publicBase = (env.ASSETS_PUBLIC_URL || "https://assets.sale.co.zw").replace(/\/+$/, "");
...
async function purgePublicCache(env, siteId) {
  try {
    ...
    const publicUrl = `https://${site.draft_subdomain}.sale.co.zw/`;
...
async function sendWhatsApp(env, phone, code) {
  ...
  try {
    ...
    const r = await fetch("https://api.manychat.com/fb/sending/sendContent", {
      ...
      body: JSON.stringify({
        subscriber_id: subId,
        data: { version: "v2", content: { messages: [{ type: "text", text: typeof code === "string" && code.length === 6 && /^\d+$/.test(code) ? `Your sale.co.zw code is: *${code}*\n\nExpires in 10 minutes.` : code }] } }
      })
    });
    return r.ok;
  } catch { return false; }
...
function applyCors(h, origin) {
  const allowed = [
    "https://app.sale.co.zw",
    "https://websites-aon.pages.dev",
    "https://www.sale.co.zw",
    "https://sale.co.zw",
    "https://sale-cozw-auth.yasibomedia.workers.dev",
  ];
